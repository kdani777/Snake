...
to proceed or exit the game. If the user proceeds, they will be instructed
to pick one of the four game modes. From there they will navigated to the game
of their choosing until they lose. Upon losing the user will be given
directions for if they want to either play again or exit the game.

***FUNCTIONS***
    title_screen
        This function formats the title screen such that the user will be given
        instructions upon start up the game
    mode_screen
        This function formats and functionalizes the mode screen so the user
        will be able to navigate between different game modes and pursue
        one of their choosing
    game_over_screen
        This function formats and functionalizes the game over screen that
        appears upon the user losing the game
'''
...
def title_screen(DISPLAYSURF, game_width):
    '''
    This function formats and functionalizes the title screen.
    On the title screen the user will see instruction on how to either
    ...
    **Return**
        Function will return itself upon being called, and all parameters
        and formatting associated with the title screen will be displayed.
    '''
    colors = get_colors()
    while True:
...
def mode_screen(DISPLAYSURF, game_width):
    ...
    their cursor. Upon clicking they will be redirected to the game mode
    ...
    **Return**
        Function will return itself upon being called, and all parameters
        and formatting associated with the mode screen will be displayed.
    '''
    colors = get_colors()
    while True:
...
def game_over_screen(DISPLAYSURF, game_width, score):
    ...
    consists of a 'Game Over' sign and the users score. They will also be
    instructed on how to play a new game or exit the the entire game. All
    actions will be conducted via keyboard keys.
    ...
    **Return**
        Function will return itself upon being called, and all parameters
        and formatting associated with the game over screen will be displayed
        in addition to the users score and instructions on how to exit the game
        or continue playing.

        play_again: *Boolean*
            This is a boolean that will either be true if user presses a key
            to play again or will return false if user opts to exit the game.
    '''
    colors = get_colors()
    while True:
        DISPLAYSURF.fill((0, 0, 0))
        ...
        final_score = font3.render('Final Score: %s' % (score), True, colors[1])
